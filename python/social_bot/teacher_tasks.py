...
class GraspingTask(teacher.Task):
    """
    A simple teacher task to grasp an object.
    For this task, the agent will receive reward 1 when it is close enough to the goal.
    If it is moving away from the goal too much or still not close to the goal after max_steps,
    it will get reward -1.
    """
    ...
    def __init__(self,
        ...
        Args:
            max_steps (int): episode will end if not reaching gaol in so many steps
            object_name (string): name of the object in the world
            success_distance_thresh (float): the goal is reached if it's within this distance to the agent
            fail_distance_thresh (float): if the agent moves away from the goal more than this distance,
                it's considered a failure and is givne reward -1
            random_range (float): the goal's random position range
    ...
    def run(self, agent, world):
        ...
        agent_sentence = yield
        agent.reset()
        target_object = world.get_agent(self._object_name)
        self._initial_loc, _ = target_object.get_pose()
        loc, _ = agent.get_pose()
        loc = np.array(loc)
        self._move_target(target_object, loc)
        steps_since_last_reward = 0
        while steps_since_last_reward < self._max_steps:
            steps_since_last_reward += 1
            loc, dir = agent.get_pose()
            target_object_loc, _ = target_object.get_pose()
            loc = np.array(loc)
            target_object_loc = np.array(target_object_loc)
            height = self._initial_loc[2]
            height_change = height - self._initial_loc[2]
            if height_change > self._success_distance_thresh:
                logger.debug("loc: " + str(loc) + " target: " +
                                str(target_object_loc) + "height_change: " + str(height_change))
                agent_sentence = yield TeacherAction(
                    reward=1.0, sentence="Well done!", done=False)
                steps_since_last_reward = 0
                self._move_target(target_object, loc)
                ...
                agent_sentence = yield TeacherAction()
        logger.debug("loc: " + str(loc) + " target: " + str(target_object_loc) +
                     "height_change: " + str(height_change))
        yield TeacherAction(reward=-1.0, sentence="Failed", done=True)

    def _move_target(self, target, agent_loc):
        range = np.array(self._random_range)
        loc = np.array(self._initial_loc) + (random.random() * range - range / 2,
            random.random() * range - range / 2, 0)
